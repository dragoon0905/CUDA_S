...
class Trainer():
    def __init__(self, cfg, logger, writer):
        ...
        self.target_loss=MaxSquareloss(ignore_index=self.ignore_index,num_class=self.cfg.data.num_classes)
    ...
    def train_one_epoch(self):
        ...
        for batch_idx, (batch_s, batch_t) in enumerate(tqdm(
            ...
            with torch.no_grad():
                ...
                pred = self.model(Variable(x).to(self.device))
                pred_1, pred_2 = unpack(pred)
                ...
                if self.cfg.aux:
                    ...
                    #label_2 = torch.where(mask, label_2, ignore_tensor)
            
            pred = self.model(Variable(x,requires_grad=True).to(self.device))
            pred_1, pred_2 = unpack(pred)
            # Loss (source)
            print(pred_1.requires_grad)
            
            loss_Ms_1 = self.target_loss(pred_1, label_1)
            if self.cfg.aux:
                loss_Ms_2 = self.target_loss(pred_2, label_2) * self.cfg.lam_aux
                loss_Ms_ = loss_Ms_1 + loss_Ms_2
            else:
                loss_Ms_ = loss_Ms_1

            # Backward
            loss_Ms_=loss_Ms_*(0.05)
            print(loss_Ms_)
            loss_Ms_.backward()

            # Clean up
            losses['Ms_main'] = loss_Ms_1.cpu().item()
            if self.cfg.aux:
                losses['Ms_aux'] = loss_Ms_2.cpu().item()
            del loss_Ms_, loss_Ms_1, loss_Ms_2
