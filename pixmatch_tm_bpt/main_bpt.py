...
class Trainer():
    def __init__(self, cfg, logger, writer):
        ...
        if self.cfg.data.target.dataset == 'cityscapes':
            ...
            target_memory_dataset=City_Dataset(split='memory_use', **self.cfg.data.target_memory.kwargs)
        ...
        self.target_memory_dataloader = DataLoader(
            target_memory_dataset, shuffle=True, drop_last=True, **self.cfg.data.loader.kwargs)
    ...
    def train(self):
        ...
        while self.continue_training:
            ...
            if self.epoch==4:
                ...
                new_params=self.model.optim_parameters(self.cfg,[1,2])
                
                self.optimizer = torch.optim.SGD(new_params,
                            lr=self.cfg.opt.learning_rate, momentum=self.cfg.opt.momentum, weight_decay=self.cfg.opt.weight_decay)
                self.optimizer.param_groups[0]["lr"]=pre_lr
    ...
    def train_one_epoch(self):
        ...
        # Training loop
        total = min(len(self.source_dataloader), len(self.target_dataloader),len(self.target_memory_dataloader))
        for batch_idx, (batch_s, batch_t,batch_m) in enumerate(tqdm(
            zip(self.source_dataloader, self.target_dataloader,self.target_memory_dataloader),
            ...
            x = x.to(self.device)
            target_img_list=[]
            target_img_list.append(x)
            loss_self_train_total=0
            if self.epoch>3:
                x_m,pred_memory,gt_m=batch_m
                target_img_list.append(x_m)
            target_i=0
            for x_ in target_img_list:
                # ##############################
                ...
                with torch.no_grad():
                    pseudo_outputs0_1, pseudo_outputs0_2 = self.model(x_.to(self.device), forward_target=0) # Source (S)
                    pseudo_outputs1_1, pseudo_outputs1_2 = self.model(x_.to(self.device), forward_target=1) # Cityscapes (T1)
                    pseudo_outputs2_1, pseudo_outputs2_2 = self.model(x_.to(self.device), forward_target=2) # IDD (T2)
                    #pseudo_outputs3_1, pseudo_outputs3_2 = self.model(x.to(self.device), forward_target=3) # 
                    if target_i>0:
                        pred_memory=pred_memory.to(self.device)
                        pseudo_outputs3_1, pseudo_outputs3_2=pred_memory,pred_memory #이전에 저장된 pred
                        ...
                        label_3_2 = torch.argmax(pseudo3_2.detach(), dim=1)
                        maxpred3_2, argpred3_2 = torch.max(pseudo3_2.detach(), dim=1)
                    ...
                    pseudo_best_23=torch.max(pseudo_best_01,pseudo2_1)
                    if target_i>0:
                    ...
                    if self.epoch>=0 and batch_idx > 0 and batch_idx % 1500 == 0:
                    
                        pseudo_ = gt_m[0].unsqueeze(0)
                        pseudo_ = pseudo_.detach().cpu().numpy()
                        pseudo_ = np.asarray(pseudo_, dtype=np.uint8).squeeze(0)
                        output_col = colorize_mask(pseudo_)
                        output_col.save('/root/test/Methods/CUDA/pixmatch_tm/t1_pseudo_or/p2_%s_%s_%s_color_gt.png' % (target_i,self.epoch,batch_idx))
                        ...
                        pseudo_ = np.asarray(pseudo_, dtype=np.uint8).squeeze(0)
                        output_col = colorize_mask(pseudo_)
                        output_col.save('/root/test/Methods/CUDA/pixmatch_tm/t1_pseudo_or/p2_%s_%s_%s_color_0.png' % (target_i,self.epoch,batch_idx))
                        #t1
                        mask_1 = (maxpred1_1 > T)
                        ignore_tensor = torch.ones(1).to(
                            self.device, dtype=torch.long) * self.ignore_index
                        ...
                        pseudo_ = np.asarray(pseudo_, dtype=np.uint8).squeeze(0)
                        output_col = colorize_mask(pseudo_)
                        output_col.save('/root/test/Methods/CUDA/pixmatch_tm/t1_pseudo_or/p2_%s_%s_%s_color_1.png' % (target_i,self.epoch,batch_idx))
                        #t2
                        mask_1 = (maxpred2_1 > T)
                        ignore_tensor = torch.ones(1).to(
                            self.device, dtype=torch.long) * self.ignore_index
                        ...
                        pseudo_ = np.asarray(pseudo_, dtype=np.uint8).squeeze(0)
                        output_col = colorize_mask(pseudo_)
                        output_col.save('/root/test/Methods/CUDA/pixmatch_tm/t1_pseudo_or/p2_%s_%s_%s_color_2.png' % (target_i,self.epoch,batch_idx))
                        #t3
                        mask_1 = (maxpred3_1 > T)
                        ignore_tensor = torch.ones(1).to(
                            self.device, dtype=torch.long) * self.ignore_index
                        ...
                        pseudo_ = np.asarray(pseudo_, dtype=np.uint8).squeeze(0)
                        output_col = colorize_mask(pseudo_)
                        output_col.save('/root/test/Methods/CUDA/pixmatch_tm/t1_pseudo_or/p2_%s_%s_%s_color_3.png' % (target_i,self.epoch,batch_idx))
                        #best
                        pseudo_ = label_1[0].unsqueeze(0)
                        pseudo_ = pseudo_.detach().cpu().numpy()
                        pseudo_ = np.asarray(pseudo_, dtype=np.uint8).squeeze(0)
                        output_col = colorize_mask(pseudo_)
                        output_col.save('/root/test/Methods/CUDA/pixmatch_tm/t1_pseudo_or/p2_%s_%s_%s_color_best.png' % (target_i,batch_idx,self.epoch))
    ################################ Pseudo2
                    ...
                    label_0_2 = torch.argmax(pseudo0_2.detach(), dim=1)
                    maxpred0_2, argpred0_2 = torch.max(pseudo0_2.detach(), dim=1)
                    
                    label_1_2 = torch.argmax(pseudo1_2.detach(), dim=1)
                    maxpred1_2, argpred1_2 = torch.max(pseudo1_2.detach(), dim=1)
                    
                    label_2_2 = torch.argmax(pseudo2_2.detach(), dim=1)
                    maxpred2_2, argpred2_2 = torch.max(pseudo2_2.detach(), dim=1)

                    
                    
                    pseudo_best_01=torch.max(pseudo0_2,pseudo1_2)
                    pseudo_best_23=torch.max(pseudo_best_01,pseudo2_2)
                    if target_i>0:
                        pseudo_best_23=torch.max(pseudo2_2,pseudo3_2)
                    pseudo_best_2=torch.max(pseudo_best_01,pseudo_best_23)

                    label_best_2 = torch.argmax(pseudo_best_2.detach(), dim=1)
                    maxpred_best_2, argpred_best_2 = torch.max(pseudo_best_2.detach(), dim=1)

                    T = 0.95
                    mask_1 = (maxpred_best > T) | (maxpred_best_2>T)
                    ignore_tensor = torch.ones(1).to(
                        self.device, dtype=torch.long) * self.ignore_index
                    label_2 = torch.where(mask_1, label_best_2, ignore_tensor)
                if target_i==0:
                    n_t=2
                elif target_i==1:
                    n_t=1

                pred = self.model(x_m.to(self.device),forward_target=n_t)
                ...
                loss_self_train_total=loss_self_train
                #loss_self_train.backward()
                del pred, pred_1_g, pred_2_g, loss_self_train, loss_self_train1, loss_self_train2,label_1,label_2
                del pseudo_outputs0_1,pseudo_outputs0_2,pseudo_outputs1_1,pseudo_outputs1_2,pseudo_outputs2_1,pseudo_outputs2_2
                del pseudo0_1,pseudo1_1,pseudo2_1,pseudo0_2,pseudo1_2,pseudo2_2
                del label_0_1,label_0_2,label_1_1,label_1_2,label_2_1,label_2_2,maxpred1_1,maxpred2_1,maxpred0_1,maxpred1_2,maxpred0_2,maxpred2_2,argpred0_1,argpred1_1,argpred2_1,argpred0_2,argpred1_2,argpred2_2
                del pseudo_best_01,pseudo_best_23,pseudo_best,label_best,maxpred_best,argpred_best,label_best_2,maxpred_best_2,argpred_best_2
                if target_i>0:
                    del pseudo_outputs3_1,pseudo_outputs3_2,label_3_1,label_3_2,pseudo3_1,pseudo3_2,maxpred3_1,maxpred3_2,argpred3_1,argpred3_2
                target_i+=1
            ##################################
            ...
            if self.cfg.lam_aug > 0:
                ...
                # Backward
                #loss_aug.backward()
                loss_new_aug = loss_self_train_total*self.cfg.lam_new + loss_aug
                ...
                del pred_aug, pred_aug_1, pred_aug_2, loss_aug, loss_aug_1, loss_aug_2,loss_self_train_total
            ...
            if self.epoch>=0 and batch_idx > 0 and batch_idx % 1500 == 0:
                ...
                # Save checkpoint if new best model
                filename=os.path.join('checkpoint_'+str(self.iter)+'pth')
                torch.save({
                    'epoch': self.epoch + 1,
                    'iter': self.iter,
