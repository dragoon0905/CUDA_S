...
class City_Dataset(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if (self.split == "train" or self.split == "trainval" or self.split=='memory') and self.training:
            image, gt_image = self._train_sync_transform(image, gt_image)
            return image, gt_image, filename
        elif (self.split=='memory_use'):
            image, gt_image = self._train_sync_transform(image, gt_image)
            pseudo_filepath=os.path.join(self.pseudo_path,filename)
            pseudo=np.load(pseudo_filepath+".npy")
            pseudo=pseudo.squeeze()
            return image,pseudo,gt_image
        else:
