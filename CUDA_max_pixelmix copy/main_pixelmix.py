...
class Trainer():
    ...
    def train_one_epoch(self):
        ...
        for batch_idx, (batch_s, batch_t) in enumerate(tqdm(
            ...
            ##########################
            x,y, _ = batch_s
            x_source = x.clone().to(self.device)
            ...
            with torch.no_grad():
                # Pseudo1
                pseudo_outputs1_1, pseudo_outputs1_2 = self.ref_model0(x.to(self.device), forward_target=1)
                pseudo_outputs2_1, pseudo_outputs2_2 = self.ref_model0(x.to(self.device), forward_target=2)
                ...
                pseudo_best = torch.max(pseudo1_1,pseudo2_1)
                pseudo_best = torch.max(pseudo_best, pseudo3_1)
                ...
                # pseudo_best2 = torch.max(pseudo1_2,pseudo2_2)
                ...
                # label_2 = torch.where(mask_2, label_best2, y)

############################### plot
            # pseudo_ = x[0]
            # # pseudo_ = torch.max(pseudo_, dim=1)[1]
            # pseudo_ = pseudo_.detach().cpu().numpy()
            # pseudo_ = np.asarray(pseudo_, dtype=np.uint8).transpose(1,2,0)
            # print(pseudo_.shape)
            # # output_col = colorize_mask(pseudo_)
            # # plt.imsave('/data/shinahyung/code/3CUDA/0530/t2_ours_pixel/p2_%s_color.png' % (batch_idx), pseudo_)
            # output_col = Image.fromarray(pseudo_).convert("RGB")
            # output_col.save('/data/shinahyung/code/3CUDA/0530/t2_ours_pixel/p2_%s_color.png' % (batch_idx))
################################
